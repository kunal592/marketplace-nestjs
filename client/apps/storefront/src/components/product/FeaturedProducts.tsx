...
export function FeaturedProducts() {
    const { data, isLoading, isError } = useQuery({
        queryKey: productKeys.list('featured'),
    ...
    if (isError || products.length === 0) {
        return (
            <div className="flex h-32 items-center justify-center text-muted-foreground w-full">
                <p>{isError ? "Failed to load products. Please ensure the backend is running." : "No new arrivals found."}</p>
            </div>
        );
    }
